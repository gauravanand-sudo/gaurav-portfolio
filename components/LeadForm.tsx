...
type Props={defaultRequest?:string;mode?:'project'|'discovery'|'referral'}
...
export default function LeadForm({defaultRequest='',mode='project'}:Props){
 ...
 useEffect(()=>{setAttribution(readAttribution() as Record<string,string>);track(mode==='project'?'contact_started':mode==='discovery'?'discovery_started':'referral_started')},[mode])
 ...
 async function submit(event:FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   await track(mode==='project'?'contact_completed':mode==='discovery'?'discovery_completed':'referral_completed',{budget:data.budget||''})
   window.location.href=mode==='project'?'/thank-you':mode==='discovery'?'/thank-you?type=discovery':'/thank-you?type=referral'
 ...
 return <form ref={formRef} className="lead-form progressive-lead-form" onSubmit={submit}>
  ...
  <fieldset data-step="1" className={`lead-fieldset ${step!==1?'lead-step-hidden':''}`}><legend>{mode==='project'?'What are you trying to accomplish?':mode==='discovery'?'What would you like help deciding?':'Who are you introducing and what do they need?'}</legend><label><span>{mode==='referral'?'Opportunity *':'Project / problem *'}</span><input name="request" required defaultValue={defaultRequest} placeholder={mode==='referral'?'Example: Referral: SaaS founder needs an MVP':'Example: launch an MVP, automate weekly reporting, rebuild our website...'}/></label><label><span>Context *</span><textarea name="message" required rows={6} placeholder={mode==='referral'?'Who is the opportunity for, what do they need, and are you authorized to share their details?':'Who is this for? What is painful today? What should be different when the work is done?'}/></label><button type="button" className="button button-primary lead-next" onClick={advance}>Continue →</button></fieldset>
